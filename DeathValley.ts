...
class DefaultOptions {
	...
	public errorMessage(code: number): string {
...
class Global {
	...
	private readonly services: Map<string, object>;
...
class Exception {
	...
	public constructor(readonly code: ErrorCode, ...args: string[]) {
		this.args = args;
...
class ServiceId<T> {
	public constructor(private readonly serviceId: string) { }
...
abstract class QueryTask extends UniqueId implements Task {
	...
	exec(): Promise<Relation[]> {
		const journal
			= this.txType === TransactionType.READ_ONLY ? undefined : new Journal(this.global, this.combinedScope);
...
class TableBuilder {
	...
	addPrimaryKey(
		columns: IndexedColumnSpec[] | string[],
		autoInc = false
	): this {
	...
	addForeignKey(name: string, rawSpec: RawForeignKeySpec): this {
		this.checkNamingRules(name);
		this.checkNameConflicts(name);
	...
	addUnique(name: string, columns: string[]): this {
		this.checkNamingRules(name);
		this.checkNameConflicts(name);
	...
	addNullable(columns: string[]): this {
